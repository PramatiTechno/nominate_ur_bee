...
def index(request):
    ...
    nominations = Nomination.objects.filter(group=requested_user_group ,nomination_timing__start_day__lte=datetime.today() + timedelta(days=1), nomination_timing__end_day__gte= datetime.today())
    new_nominations = []
