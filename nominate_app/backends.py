...
class NominateCASBackend(ModelBackend):
    """CAS authentication backend"""
    ...
    def authenticate(self, request, ticket, service):
        ...
        if attributes['jobtitle'] not in self.title_role_map:

            HttpResponseRedirect(reverse('cas_ng_login')) # or http response
            return None
        else:
            ...
            if settings.CAS_APPLY_ATTRIBUTES_TO_USER and attributes:
                ...
                if settings.CAS_CREATE_USER:
                    user.save()
                    user_profile_model_fields = UserProfile._meta.fields
                    userprofile = {}
                    for field in user_profile_model_fields:
                    # Handle null -> '' conversions mentioned above
                        if not field.null:
                            try:
                                if attributes[field.name] is None:
                    ...
                    # Coerce boolean strings into true booleans
                        if field.get_internal_type() == 'BooleanField':
                            try:
                                ...
                                continue
                        userprofile[field.name] = attributes[field.name]
